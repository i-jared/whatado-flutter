...
class CloudStorageService {
  ...
  Future<String?> uploadImage(Uint8List imageData, int userId,
      {bool userImage = false}) async {
    firebase_storage.Reference ref = storage.ref(
        '${userImage ? 'user_images' : 'event_images'}/${userId}_${DateTime.now()}.png');
    try {
  ...
  Uint8List resize({required File? file, int x = 600, int y = 600}) {
    ...
    final resizedFace =
        fi.copyResize(decodedImage as fi.Image, height: y, width: x);
